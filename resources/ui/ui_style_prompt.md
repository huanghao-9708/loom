...
## 🧩 七、多标签页与沙盒工作区架构 (Multi-Tab & Sandbox Workspace)

Loom 在交互上不只是一个单页应用，而是**大平层级的多任务控制中心**，必须遵循以下动态布局架构：

### 1️⃣ 顶部全局 Tab Bar
* 点击左侧导航（文件源、插件等）不应当覆盖原有界面，而应在顶部生成类似浏览器的**独立标签页 (Tab)**。
* 必须支持多个插件与文件视图的并行流切换，标签需带有关闭 (`x`) 交互。

### 2️⃣ 动态上下文看板 (Contextual Right Panel)
* **退让原则**：右侧信息看板（如“存储预览”与“文件分类统计”）**仅在激活的 Tab 为“全部文件”或原生资源管理器时显示**。
* **沉浸原则**：当激活的 Tab 为插件（如音乐播放器、阅读器等）时，右侧看板必须完全折叠/隐藏，将底部与右侧全部横向与纵向空间 **100% 留给插件工作区**。

### 3️⃣ 驻留式沙盒 (KeepAlive Plugins)
* 插件界面 (如 IFrame 容器) 应当通过 `v-show` 机制常驻 DOM，以确保在不同 Tab 之间切换时，插件的状态（如音乐播放进度、复杂表单）绝不会因为页面销毁而中断。